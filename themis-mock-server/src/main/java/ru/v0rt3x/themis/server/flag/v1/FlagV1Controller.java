...
import ru.v0rt3x.themis.server.flag.FlagCollector;
...
public class FlagV1Controller {
    ...
    public ResponseEntity<Integer> submitFlag(@RequestBody String flag, HttpServletRequest request) {
        ...
        SubmitResult result = SubmitResult.ERROR_FLAG_NOT_FOUND;

        if (flagCollector.hasFlag(flag)) {
            result = SubmitResult.SUCCESS;
            flagCollector.removeFlag(flag);
        } else {
